...
def plot_accuracy():
    ...
    # plt.plot(x,y1,'-',label='No Adversarial Clients')
...
def plot_time_to_accuracy():
    ...
    y4 = np.around(read_accuracy('cs4.txt', 'Test Epoch', '[-+]?[0-9]+\.+[0-9]+')[0:101,2], decimals=3)
    x5 = np.around(read_accuracy('cs5-400epochs.txt', 'time cost', '[-+]?[0-9]+\.+[0-9]+')[0:101,0], decimals=3)
    y5 = np.around(read_accuracy('cs5-400epochs.txt', 'Test Epoch', '[-+]?[0-9]+\.+[0-9]+')[0:101,2], decimals=3)
    x6 = np.around(read_accuracy('cs6-400epoch.txt', 'time cost', '[-+]?[0-9]+\.+[0-9]+')[0:101,0], decimals=3)
    y6 = np.around(read_accuracy('cs6-400epoch.txt', 'Test Epoch', '[-+]?[0-9]+\.+[0-9]+')[0:101,2], decimals=3)

    plt.plot(x2,y2,'-',label='No Client Selection')
...
def plot_detection_rate():
    ...
    y_label4, y_pre4 = read_roc('cs4.txt','logger mean','real list is','[-+]?[0-9]+\.+[0-9]+','[-+]?[0-9]+')
    fpr4, tpr4, thersholds4 = roc_curve(y_label4, y_pre4, pos_label=1)
    roc_auc4 = auc(fpr4, tpr4)
    # for i, value in enumerate(thersholds):
        # print("%f %f %f" % (fpr[i], tpr[i], value))
    y_label5, y_pre5 = read_roc('cs5.txt','output is','real list is','[-+]?[0-9]+\.+[0-9]+','[-+]?[0-9]+')
    fpr5, tpr5, thersholds5 = roc_curve(y_label5, y_pre5, pos_label=1)
    roc_auc5 = auc(fpr5, tpr5)

    y_label6, y_pre6 = read_roc('cs6.txt','output1 is','real list is','[-+]?[0-9]+','[-+]?[0-9]+')
